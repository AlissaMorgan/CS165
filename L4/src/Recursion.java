...
public class Recursion {
     ...
     */
    public static int longestWordHelper(Scanner scanner, int longest){
        if(!scanner.hasNext()){return longest;}
        String cur = scanner.next().replaceAll("[^a-zA-Z]", "");
        if(cur.length() > longest){
            longest = cur.length();
        }
        return longestWordHelper(scanner, longest);
    }
    public static int longestWordLength(String words){
        Scanner scnr = new Scanner(words);
        return longestWordHelper(scnr, 0);
    }
...
//        if(word.length() == 2){return word.substring(0, 1);}
        if(word.length() >= 2 && Character.toLowerCase(word.charAt(0)) == Character.toLowerCase(word.charAt(1))){
    ...
    public static void main(String [] args){
      ...
      System.out.printf("isPalindrome(\"Recursion\") should be false -> %b\n", isPalindrome("Recursion"));
        ...
        System.out.printf("dedupeChars(\"Ppi\") should be Pi -> %s\n", dedupeChars("Ppi"));
